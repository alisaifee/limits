...
def test_fixed_window_with_elastic_expiry_in_memory():
    storage = MemoryStorage()
...
def test_fixed_window_with_elastic_expiry_memcache_concurrency():
    storage = MemcachedStorage('memcached://localhost:22122')
    limiter = FixedWindowElasticExpiryRateLimiter(storage)
    start = int(time.time())
    limit = RateLimitItemPerSecond(10, 2)
...
def test_fixed_window_with_elastic_expiry_redis_sentinel():
...
def test_moving_window_in_memory():
    storage = MemoryStorage()
